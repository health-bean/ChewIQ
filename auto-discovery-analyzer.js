...
// Allow custom lambda path via command line
if (args.includes('--lambda-path')) {
  const pathIndex = args.indexOf('--lambda-path');
  if (pathIndex + 1 < args.length) {
    options.lambdaPath = args[pathIndex + 1];
  }
}

// Run the auto-discovery analysis
const analyzer = new AutoDiscoveryAPIAnalyzer(options);
